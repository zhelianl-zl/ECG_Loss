...
Usage examples:
  ...
  # Evaluate all checkpoints in a directory (auto-detect dataset from filename)
  ...
  # Evaluate with C-suite and LT metrics
  python tools/eval_checkpoints.py --ckpt_dir models/ --pattern "*cifar10*" \
      --c_corruptions gaussian_noise,brightness --c_severity 5 \
      --imbalance exp --imb_factor 100 --imb_seed 0
...
# Add parent dir to path so we can import models, ecg_loss, train, etc.
...
# Reuse SmallImagenet from train.py if needed
def _get_small_imagenet_class():
    from train import SmallImagenet
    return SmallImagenet


def build_test_loader(dataset_name, batch_size=32, imbalance="none", imb_factor=None, imb_seed=None):
    """Build test DataLoader (and optionally train for LT). Returns (test_loader, train_loader_or_None, num_classes, test_transform)."""
    train_loader = None
...
def eval_corruption(model, device, dataset_name, corruption, severity, batch_size=32):
    """Evaluate on CIFAR-10-C or CIFAR-100-C."""
    ...
    with torch.no_grad():
        for X, y in c_loader:
            X, y = X.to(device), y.to(device)
            total_err += (model(X).argmax(1) != y).sum().item()
            n += len(y)
    return total_err / n if n > 0 else None
...
def eval_longtail(model, loader, device, num_classes):
    ...
    return {"balanced_acc": balanced, "many_acc": many, "medium_acc": medium, "few_acc": few}
...
def run_eval(args):
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            name=args.wandb_name or f"eval_{ds}_{time.strftime('%m%d_%H%M')}",
            group=args.wandb_group or "eval",
            config=vars(args),
        )
    ...
    for epoch, ckpt_path in ckpts:
        ...
        for atk in attacks:
            ...
            print(f"  {atk:15s}  Error={adv_err:.4f}  Loss={adv_loss:.4f}  ({dt:.1f}s)")

        # C-suite
        for corr in corruptions:
            c_err = eval_corruption(model, device, ds, corr, args.c_severity, args.batch_size)
            if c_err is not None:
                row[f"C/{corr}/s{args.c_severity}/Error"] = c_err
                print(f"  C/{corr}/s{args.c_severity}  Error={c_err:.4f}")

        # LT
        if args.imbalance and args.imbalance != "none":
            lt = eval_longtail(model, test_loader, device, num_classes)
            for k, v in lt.items():
                row[f"LT/{k}"] = v
            print(f"  LT  BalAcc={lt['balanced_acc']:.4f}  Many={lt['many_acc']:.4f}  Med={lt['medium_acc']:.4f}  Few={lt['few_acc']:.4f}")
...
def main():
    ...
    parser.add_argument("--pattern", type=str, default="*_epoch*.pt", help="Glob pattern for checkpoints in ckpt_dir.")
    parser.add_argument("--dataset", type=str, default=None, choices=["cifar10", "binaryCifar10", "cifar100", "svhn", "imageNet"],
